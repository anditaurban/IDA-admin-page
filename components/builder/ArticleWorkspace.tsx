...
const ToolbarButton = ({ onClick, isActive, icon, title }: { onClick: () => void, isActive: boolean, icon: string, title: string }) => (
  <button 
    ...
    title={title} 
    className={`size-8 md:size-9 flex items-center justify-center rounded-full transition-all duration-200 
      ${isActive 
        ? 'bg-[#00BCD4] text-white shadow-md shadow-cyan-500/30 scale-105' 
  ...
  return (
    <div className="sticky top-6 z-30 mx-auto w-max max-w-[95%] bg-white/80 dark:bg-[#1b2636]/80 backdrop-blur-xl border border-slate-200/50 dark:border-slate-700/50 p-1.5 rounded-full shadow-2xl shadow-slate-200/50 dark:shadow-black/50 flex items-center gap-1 overflow-x-auto no-scrollbar ring-1 ring-slate-900/5 dark:ring-white/5">
      <button 
         ...
         className={`px-4 py-1.5 text-xs md:text-sm font-bold rounded-full transition-all flex items-center gap-1.5 
            ${editor.isActive('heading', { level: 2 }) 
                ? 'bg-[#00BCD4] text-white shadow-md shadow-cyan-500/30' 
...
export default function ArticleWorkspace({ title, setTitle, initialContent, onContentUpdate, onSave, onDelete }: ArticleWorkspaceProps) {
  ...
  }, [localTitle, adjustTextareaHeight]);

  let cleanContent = initialContent;
  if (cleanContent && cleanContent.includes("Mulai ketik materi")) {
      cleanContent = ""; 
  }
        ...
        <div className="flex items-center justify-between mb-8">
           ...
           <div className="flex items-center gap-3">
              ...
              <button 
                 ...
                 className={`flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-bold transition-all active:scale-95 ${isDirty ? 'bg-amber-400 hover:bg-amber-500 text-slate-900 shadow-amber-400/20 animate-pulse' : 'bg-[#00BCD4] hover:bg-[#00acc1] text-white shadow-lg shadow-cyan-500/20'}`}
              >
