...
function normalizeStringArray(value: unknown): string[] {
  ...
  if (typeof value === "string") {
    ...
    try {
      ...
      // Jika bukan JSON array, gunakan pemisah koma.
    }
...
function CourseEditorContent() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "materials" | "videos" | "assignments" | "reviews"
  >("overview");
  ...
  useEffect(() => {
    ...
    const fetchCourseDetail = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${BASE_URL}/detail/course/${courseSlug}`,
          {
            method: "GET",
            headers: {
        ...
        if (response.ok) {
          ...
          if (resJson.tableData && Array.isArray(resJson.tableData)) {
            apiData =
              resJson.tableData.find(
                (c: Record<string, unknown>) =>
                  String(c.course_id) === String(courseSlug),
              ) || resJson.tableData[0];
        ...
        if (
          ...
          showToast(
            "error",
            "Koneksi ke server terputus. Pastikan server backend Anda menyala.",
          );
        }
      } finally {
        setIsFetching(false);
    ...
    fetchCourseDetail();
  }, [courseSlug, getActiveToken, handleAuthError, showToast]);
  ...
  const handleThumbnailUpload = async (
    ...
    if (!file.type.startsWith("image/")) {
      showToast(
        "error",
        "Format file tidak didukung. Harap pilih gambar (JPG/PNG).",
      );
    ...
    if (file.size > MAX_SIZE_MB * 1024 * 1024) {
      showToast(
        "error",
        `Ukuran gambar terlalu besar! Maksimal ${MAX_SIZE_MB}MB.`,
      );
      if (e.target) e.target.value = "";
      return;
    }
    ...
    try {
      ...
      // ✨ TANGKAP PENOLAKAN DARI BACKEND ✨
      if (resData.data?.success === false || resData.success === false) {
        const backendMsg =
          resData.data?.message ||
          resData.message ||
          "Ditolak oleh Server Backend";
        throw new Error(`Server API: ${backendMsg}`);
      }
      ...
      const uploadedUrl =
        ...
        resData.detail?.thumbnail ||
        resData.data?.file_url;

      if (uploadedUrl) {
        // Menyimpan apapun yang dibalas server (baik full URL atau sekedar nama file)
        handleBasicChange("thumbnail", uploadedUrl);
        showToast("success", "Gambar berhasil diunggah ke server!");
      } else {
        console.error("Isi JSON dari Backend:", resData);
        const resString = JSON.stringify(resData).substring(0, 90);
        throw new Error(
          `Sukses (200), tapi tanpa URL gambar. Balasan: ${resString}...`,
        );
      }
    } catch (error) {
      ...
      if (
        ...
        showToast(
          "error",
          "Koneksi jaringan gagal. Pastikan server API aktif dan mengizinkan CORS.",
        );
        ...
        showToast(
          "error",
          "Gagal memproses gambar. Pastikan endpoint server tersedia.",
        );
      }
    } finally {
  ...
  const handlePublishToServer = async () => {
    ...
    try {
      ...
      if (response.status === 401 || response.status === 403) {
        showToast(
          "error",
          "Gagal update ke server (401). Pastikan token valid.",
        );
        return;
      }
      ...
      if (response.ok) {
        ...
        showToast(
          "success",
          "Perubahan berhasil disimpan & dipublikasikan ke Server!",
        );
      } else {
      ...
      if (
        ...
        showToast(
          "error",
          "Koneksi gagal. Periksa status server Backend Anda.",
        );
      } else {
        showToast(
          "error",
          "Gagal menyambung ke server API. Pastikan endpoint tersedia.",
        );
      }
    } finally {
      setIsSaving(false);
  ...
  const parsedPrice = parseInt(
    String(basicData.price).replace(/\D/g, "") || "0",
    10,
  );
  const parsedDiscount = parseInt(
    String(basicData.discount).replace(/\D/g, "") || "0",
    10,
  );
  ...
  const displayThumbnail = React.useMemo(() => {
    ...
    // Jika backend hanya membalas nama file, kita rakit URL-nya menggunakan endpoint get-thumbnail
    const cleanPath = thumb.startsWith("/") ? thumb.slice(1) : thumb;
    return `${BASE_URL}/thumbnail/course/${cleanPath}`;
  }, [basicData.thumbnail]);

  const isImageThumbnail =
    displayThumbnail.startsWith("http") ||
    displayThumbnail.startsWith("data:image") ||
    displayThumbnail.startsWith("/");
  ...
  const resolvedCourseId =
    basicData.course_id ||
    (Number.isFinite(Number(courseSlug)) ? Number(courseSlug) : undefined);
        ...
        <div className="bg-white dark:bg-[#111111] p-8 rounded-4xl border border-slate-200/80 dark:border-slate-800/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.1)] flex flex-col lg:flex-row gap-10 mb-10">
          ...
          <div className="w-full lg:w-85 shrink-0">
            ...
            >
              ...
              {displayThumbnail ? (
                <div
                  className={`absolute inset-0 bg-cover bg-center transition-transform duration-700 group-hover:scale-105 ${isCssThumbnail ? displayThumbnail : ""}`}
                  style={
                    isImageThumbnail
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div
              className={`flex flex-wrap items-center gap-2 mb-8 ${isFetching ? "opacity-50" : ""}`}
            >
            ...
            <div className="mt-auto grid grid-cols-1 sm:grid-cols-3 gap-4">
              {/* Level */}
              <div
                className={`flex flex-col gap-1.5 p-4 rounded-2xl bg-[#fafafa] dark:bg-[#161616] border border-slate-200/60 dark:border-slate-800/60 focus-within:border-[#00BCD4]/50 focus-within:ring-2 focus-within:ring-[#00BCD4]/10 transition-all ${isFetching ? "opacity-50 animate-pulse" : ""}`}
              >
              ...
              {/* Price */}
              <div
                className={`flex flex-col gap-1.5 p-4 rounded-2xl bg-[#fafafa] dark:bg-[#161616] border border-slate-200/60 dark:border-slate-800/60 focus-within:border-[#00BCD4]/50 focus-within:ring-2 focus-within:ring-[#00BCD4]/10 transition-all ${parsedDiscount > 0 ? "row-span-2 sm:row-span-1" : ""} ${isFetching ? "opacity-50 animate-pulse" : ""}`}
              >
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                  <span className="material-symbols-outlined text-[14px]">
                    payments
              ...
              {/* Discount */}
              <div
                className={`flex flex-col gap-1.5 p-4 rounded-2xl bg-rose-50/50 dark:bg-rose-500/5 border border-rose-100 dark:border-rose-500/20 focus-within:border-rose-300 dark:focus-within:border-rose-500/50 focus-within:ring-2 focus-within:ring-rose-500/10 transition-all ${isFetching ? "opacity-50 animate-pulse" : ""}`}
              >
                ...
                <div className="relative flex items-center">
                  <input
                    ...
                    placeholder="0"
                  />
                  <span className="text-sm font-bold text-rose-500/50 pointer-events-none">
                    %
                  </span>
                </div>
              </div>
